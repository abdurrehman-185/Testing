...
function readableSupabaseError(error) {
  ...
  if (error.message === "Failed to fetch" || error.name === "TypeError") {
    return "Could not reach Supabase. Check NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY in .env.local or Vercel Environment Variables.";
  }
...
export default function DashboardPage() {
  ...
  function clearTracks() {
    ...
    setErrorMessage("");
    setLiveStatus(
      liveEnabled ? "Tracks cleared - waiting for next GPS point" : "Tracks cleared"
    );
  }
  ...
  async function fetchLatestLivePoints() {
    ...
    setRoutePoints((currentPoints) =>
      mergeRoutePoints(currentPoints, latestPoints)
    );
    setLiveStatus(
      `Live: ${new Date(newestPoint.timestamp).toLocaleTimeString()}`
    );
  }
  ...
  function startLivePolling() {
    ...
    livePollIntervalRef.current = window.setInterval(
      fetchLatestLivePoints,
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          if (
            wasClearedAfter &&
            new Date(nextPoint.timestamp) < new Date(wasClearedAfter)
          ) {
            return;
          }

          setRoutePoints((currentPoints) =>
            mergeRoutePoints(currentPoints, [nextPoint])
          );

          setLiveStatus(
            `Live: ${new Date(nextPoint.timestamp).toLocaleTimeString()}`
          );
        }
      )
      .subscribe((status) => {
        ...
        <section className="rounded-lg border border-stone-200 bg-white p-4 shadow-sm">
          <div className="grid gap-4">
            <label className="grid gap-2 text-sm font-semibold text-zinc-800">
              Start Time
              <input className="rounded-md border border-stone-300 bg-white px-3 py-2 text-sm font-medium text-zinc-950 outline-none ring-blue-600 transition focus:ring-2" type="datetime-local" value={startTime} onChange={(event) => setStartTime(event.target.value)} />
            </label>

            <label className="grid gap-2 text-sm font-semibold text-zinc-800">
              End Time
              <input className="rounded-md border border-stone-300 bg-white px-3 py-2 text-sm font-medium text-zinc-950 outline-none ring-blue-600 transition focus:ring-2" type="datetime-local" value={endTime} onChange={(event) => setEndTime(event.target.value)} />
            </label>

            <button className="rounded-md bg-blue-700 px-4 py-2.5 text-sm font-bold text-white transition hover:bg-blue-800 disabled:cursor-not-allowed disabled:bg-zinc-400" disabled={loading} onClick={fetchRoute} type="button">
              {loading ? "Fetching..." : "Fetch Route"}
            </button>

            <button className={`rounded-md px-4 py-2.5 text-sm font-bold text-white transition disabled:cursor-not-allowed disabled:bg-zinc-400 ${liveEnabled ? "bg-red-600 hover:bg-red-700" : "bg-emerald-700 hover:bg-emerald-800"}`} disabled={loading} onClick={liveEnabled ? () => stopLiveTracking() : startLiveTracking} type="button">
              {liveEnabled ? "Stop Live Tracking" : "Start Live Tracking"}
            </button>

            <button className="rounded-md border border-stone-300 bg-white px-4 py-2.5 text-sm font-bold text-zinc-800 transition hover:bg-stone-100 disabled:cursor-not-allowed disabled:text-zinc-400" disabled={routePoints.length === 0} onClick={clearTracks} type="button">
              Clear Tracks
            </button>

            <p className="rounded-md border border-stone-200 bg-stone-50 px-3 py-2 text-sm font-semibold text-zinc-700">
              {liveStatus}
            </p>
